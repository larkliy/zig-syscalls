...
fn getSyscallCodeByName(self: *Self, comptime sys_name: []const u8) !u32 {
    comptime {
        if (!std.mem.startsWith(u8, sys_name, "Zw") and !std.mem.startsWith(u8, sys_name, "Nt")) {
...
pub fn doSyscall(self: *Self, comptime name: []const u8, args: anytype) usize {
    var abi_args: [10]usize = .{0} ** 10;
    inline for (args, 0..) |arg, i| {
        abi_args[i] = toUsize(arg);
    }

    const ssn = self.getSyscallCodeByName(name) catch return 0;
